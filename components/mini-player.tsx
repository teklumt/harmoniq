    ...
    <div className="fixed bottom-20 left-0 right-0 bg-background/95 backdrop-blur-sm border-t px-4 py-3">
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <Button variant="ghost" size="icon" className="flex-shrink-0" onClick={() => toggleLike(currentTrack.id)}>
            <Heart className={`h-4 w-4 ${likedTracks.has(currentTrack.id) ? "fill-red-500 text-red-500" : ""}`} />
          </Button>
        </div>
